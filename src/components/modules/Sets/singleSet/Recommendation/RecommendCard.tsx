...
export default function RecommendCard({ src, alt, hero, setName, price }: data) {
    return (
        <Link
            ...
            className="mx-2 h-full relative mt-6 flex flex-col items-stretch rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 sm:shrink-0 sm:grow sm:basis-0">
            <>
                <Image
                    className="rounded-t-lg transition-opacity h-[352.55px] object-cover w-full"
                    src={src}
                    alt={alt}
                    width={317}
                    height={211}
